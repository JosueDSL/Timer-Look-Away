...
look_sound = LoadSound("/home/joxulds/personal-look-away/look_away.wav").load()
break_sound = LoadSound("/home/joxulds/personal-look-away/take-a-break.wav").load()
...
while True:
    ...
    if take_a_break.counter >= take_a_break.interval:
        look_away.reset()        
        with open("/home/joxulds/personal-look-away/time_check.txt", "a") as file:
            update = file.write("Break 2H Played at: " + time.strftime("%H:%M:%S") + "\n")
        take_a_break.reset()
        if break_sound:
            break_sound.play()
            pygame.time.delay(int(break_sound.get_length() * 1000))  # Delay takes input in milliseconds
        else:
    ...
    if look_away.counter >= look_away.interval:
        ...
        if look_sound:
            look_sound.play()
            pygame.time.delay(int(look_sound.get_length() * 1000))  # Delay takes input in milliseconds
        else:
